const regex = /p[\doòóôõö@]+(rr|h)[\daàáâãä@]|p[\doòóôõö@](rr|h)[\S]+nh[\S]|(c|k)[\daáàâãä@]+r[\daáàâãä@]+lh[\S]+|\b(c|k)[uùúüû]+(\s|\n|$|!|\.|\?)\b|\b(c|k)[uùúüû]+[sz]+[\diìíïaâãàáä@]+[\S]+|b[\duùúüûoòóõôö]+(c|s)+[\deèéêë]+t+[\diìíîïaâãàá@]+[\S]+|p[\diìíïî]+(c|k|ck)+[\daàáäâ@]|p[\diìíïî]+r[\doóòöô@]+(c|k)[\S]+|\bp[uùúüû#@]+t[\daàáäâãiìíîïoóòôõôeêéèë@]+(r|n)*[\S]*|(c|k)[\daàáâãä@](c|s)[\deèéêë&]+t[\S]|(c|k)[\daàáâãä@]7|p[\deèéêë]*r[\deèéêë]+r[\deèéêë]+(c|k)[\s\S]{1}|f[\doòóôõöuùúüû@]+d[\S]+r|(x|ch)[\S]+(x|ch)[\S]+t[\S]+|(x|ch)[\doòóôõö@]+t[\daáàãâä@]+|b[\doòóõôö@]+(qu|k)[\S]+t[\S]+|p[\deéèêë&]+n+[\diíìîï]+s|\b[\daàáâãä@]+n+[\daàáâãä@]+(u|l)+\b|[\daàáâãä@]+n+[uùúûü]+(s|$)|\b[\doòóôõö@]+t+[\daàáâãä@]+r+[\diíìîï]+[\doóòôõö@]+|b[\S]b[\S](c|K)(\S|$|\s)|pr[\S]+(x|ch)[\S]+(c|k)(\S|$|\s|\n)|\bp[uúùûü]+s+y\b|(x|ch)[\S]+b+[\S]+(c|k)|(x|ch)[\daàáâãä@]+n+[\daàáâãä@]+|b[\doòóôõö@]+g+[\daàáâãä@]+|m[\deèéêë&]+r+d+[\S]+|f[\doóòõôöuúùûü@]+d+[\S]+|f[\duúùüûoòóôöõ@]+ck|d[\diìíîïi]+ck|\bv?tnc\b|\bpqp\b|\bvsf\b|\bkct\b|\bpnc\b|\bfdp\b|\b(c|k)rlh?\b/gi
